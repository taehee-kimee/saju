...
const OHAENG_LIST = ['木', '火', '土', '金', '水'];
const GROUP_LIST = ['NT', 'NF', 'ST', 'SF'];
...
const CAT_NAMES = {
  "NT_木": "탐정냥", "NT_火": "보스냥", "NT_土": "철학냥", "NT_金": "냉미냥", "NT_水": "음모냥",
  "NF_木": "새벽냥", "NF_火": "열정냥", "NF_土": "공감냥", "NF_金": "순백냥", "NF_水": "신비냥",
  "ST_木": "믿음냥", "ST_火": "돌격냥", "ST_土": "든든냥", "ST_金": "완벽냥", "ST_水": "침착냥",
  "SF_木": "따뜻냥", "SF_火": "인싸냥", "SF_土": "집사냥", "SF_金": "섬세냥", "SF_水": "치유냥"
};
...
function parseMarkdownSections(content) {
  ...
  while ((match = h2Regex.exec(content)) !== null) {
    sectionTexts.push({ title: match[1], start: match.index + match[0].length });
  }

  for (let i = 0; i < sectionTexts.length; i++) {
    const start = sectionTexts[i].start;
    const end = i + 1 < sectionTexts.length ? sectionTexts[i + 1].start - sectionTexts[i + 1].title.length - 4 : content.length;
    const text = content.substring(start, end).trim().replace(/\n+/g, ' ').replace(/\s+/g, ' ').trim();
    const title = sectionTexts[i].title;

    if (title.includes('🐾')) sections.diagnosis = text;
    else if (title.includes('🧭')) sections.ohaengMap = text;
    else if (title.includes('🧩')) sections.combination = text;
    else if (title.includes('🔄')) sections.pattern = text;
    else if (title.includes('🚨')) sections.alert = text;
  }

  return sections;
}

function readCharacterFile(mbtiType, ohaeng) {
  const folder = mbtiType.toLowerCase();
  const ohaengEncoded = ohaeng;
  const filePath = path.join(__dirname, '..', 'data', 'characters', folder, `${mbtiType}_${ohaengEncoded}.md`);
...
for (const group of GROUP_LIST) {
  for (const ohaeng of OHAENG_LIST) {
    const catId = `${group}_${ohaeng}`;
    const mbtiType = GROUP_REPRESENTATIVE[group];
    const content = readCharacterFile(mbtiType, ohaeng);
    ...
    const sub = subtitles[catId] || {};
    const name = CAT_NAMES[catId] || catId;
    const emoji = CAT_EMOJIS[catId] || '🐱';
    const tagline = taglines[catId] || '';
    const patternContent = patternContents[catId] || '';
    const mission7 = gaeunbypMission7[ohaeng] || [];

    ts += `  "${catId}": {
    id: "${catId}",
    name: "${escapeStr(name)}",
    emoji: "${escapeStr(emoji)}",
    ...
    sections: {
      diagnosis: "${escapeStr(parsedSections.diagnosis || '')}",
      ohaengMap: "${escapeStr(parsedSections.ohaengMap || '')}",
      mbtiEngine: "${escapeStr(parsedSections.combination || '')}",
      combination: "${escapeStr(parsedSections.combination || '')}",
      pattern: "${escapeStr(patternContent)}",
      love: "${escapeStr(defaultFortunes.love)}",
      money: "${escapeStr(defaultFortunes.money)}",
      career: "${escapeStr(defaultFortunes.career)}",
      health: "${escapeStr(defaultFortunes.health)}",
      relationship: "${escapeStr(defaultFortunes.relationship)}",
      mission7: [${mission7.map(x => `"${escapeStr(x)}"`).join(', ')}]
    }
  },
`;
  }
...
export const CAT_IDS = [
  ${GROUP_LIST.flatMap(g => OHAENG_LIST.map(o => `"${g}_${o}"`)).join(', ')}
];
`;

const outPath = path.join(__dirname, '..', 'data', 'catContents.ts');
const outDir = path.dirname(outPath);
if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
fs.writeFileSync(outPath, ts, 'utf-8');
console.log(`Generated catContents.ts with ${GROUP_LIST.length * OHAENG_LIST.length} characters`);
