...
export default function PaymentPage() {
  ...
  return (
    <main className="min-h-screen p-6 max-w-md mx-auto">
      <button
        onClick={() => router.back()}
        className="mb-6 text-orange-400 hover:text-orange-800 text-sm flex items-center gap-1"
      >
        ← 뒤로
      </button>

      <h1 className="text-2xl font-light mb-2 font-title">풀리포트 구매</h1>
      <p className="text-orange-600 mb-6">냥세 풀리포트를 확인해보세요!</p>

      <div className="bg-secondary rounded-2xl p-5 mb-4">
        <div className="flex justify-between items-center mb-2">
          <span className="text-orange-800">상품명</span>
          <span className="font-medium">냥세 풀리포트</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-orange-800">금액</span>
          <span className="text-xl font-bold text-primary">1,900원</span>
        </div>
      </div>

      <div className="bg-gray-50 rounded-2xl p-4 mb-6">
        <h3 className="font-bold text-orange-800 mb-3">포함 내용</h3>
