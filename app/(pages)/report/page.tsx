...
function ReportContent() {
  ...
  return (
    <main className="min-h-screen p-6 pb-32 max-w-md mx-auto">
      ...
      {/* ── 오행 바 + MBTI ── */}
      <div className="mb-6">
        <h3 className="font-bold mb-3 text-gray-600">나의 오행 에너지</h3>
            ...
            <div className="grid grid-cols-5 gap-1.5">
              {([
                ...
                <div key={g} className="flex flex-col items-center bg-white rounded-xl px-1 py-2 border border-slate-200">
                  ...
                  <span className="text-xl font-bold text-gray-900 my-0.5">{p.tenGodChart.groupSummary[g]}</span>
      ...
      {p?.dayMasterStrength && p?.favorableElement && (
        <section id="sec-strength" className="py-10 border-b border-dashed border-gray-100 scroll-mt-4 last-of-type:border-0">
          ...
          <div className="bg-slate-50 rounded-2xl p-4 space-y-3 border-t-4 border-green-300">
            ...
            <p className="text-gray-600 text-sm leading-relaxed">{p.dayMasterStrength.analysis}</p>
                ...
                <div className="bg-slate-50 rounded-2xl p-4 border-l-4 border-slate-400">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xl font-bold text-gray-600">{curr.stem}{curr.branch}</span>
                      <span className="text-xs bg-slate-200 text-gray-600 px-2 py-0.5 rounded-full">{curr.tenGod} 대운</span>
                    </div>
                    <span className="text-xs text-gray-400">{curr.startAge}~{curr.endAge}세</span>
                  </div>
                  <p className="text-sm text-gray-600 leading-relaxed mb-2">{curr.theme}</p>
                  {remainingYears !== null && (
                    <div className={`text-xs px-3 py-1.5 rounded-full inline-block font-medium ${isLateHalf ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-gray-600'}`}>
                      {isLateHalf
                  ...
                  <div className="bg-violet-50 rounded-2xl p-4 border-l-4 border-violet-400">
                    ...
                    {curr && (
                      <div className="bg-white rounded-xl p-3 text-xs text-gray-600 leading-relaxed">
                        <span className="font-bold text-gray-600">성향 변화 포인트 ✦</span><br />
                        지금의 <span className="text-gray-600 font-bold">{curr.tenGod}</span> 에너지 중심에서,
            ...
            {p.repeatPatterns.map((rp, i) => (
              <div key={i} className="bg-white rounded-xl p-3 border border-slate-200">
                <div className="flex items-start gap-2 mb-2">
                  ...
                  <p className="text-gray-900 font-medium text-sm leading-snug">&ldquo;{rp.pattern}&rdquo;</p>
                </div>
            ...
            {fortunes[activeFortuneTab] === null ? (
              ...
              <div className="space-y-4">
                {(fortunes[activeFortuneTab] as string)
                  ...
                  .map((para, i) => (
                    <p key={i} className="text-gray-600 text-[15px] leading-[2] whitespace-pre-line">
                      {para}
                    </p>
                  ))}
              </div>
        ...
        return (
          <section id="sec-strategy" className="py-10 border-b border-dashed border-gray-100 scroll-mt-4 last-of-type:border-0">
            ...
            <div className="bg-slate-800 text-slate-100 rounded-2xl p-4 mb-4 space-y-2">
              <p className="text-xs text-gray-400 mb-2 font-medium">왜 이 전략이 필요한가</p>
              ...
              <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
                ...
                <div className="space-y-2">
                  <div className="flex gap-2 items-center">
                    ...
                    <span className="text-sm text-gray-600">{strategy.env.color}</span>
                  </div>
                  <div className="flex gap-2 items-center">
                    <span className="text-xs text-amber-600 w-12 shrink-0">공간</span>
                    <span className="text-sm text-gray-600">{strategy.env.space}</span>
                  </div>
                  <div className="flex gap-2 items-center">
                    <span className="text-xs text-amber-600 w-12 shrink-0">시간대</span>
                    <span className="text-sm text-gray-600">집중 작업에 유리한 {strategy.env.time}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-violet-50 rounded-2xl p-4 border border-violet-100">
                ...
                <div className="space-y-2">
                  <div className="flex gap-2 items-start">
                    ...
                    <span className="text-sm text-gray-600">{strategy.positiveRelation}</span>
                  </div>
                  <div className="flex gap-2 items-start">
          ...
          <section id="sec-outlook" className="py-10 border-b border-dashed border-gray-100 scroll-mt-4 last-of-type:border-0">
            ...
            <div className="space-y-3">
              {saju.payload.threeYearOutlook.map((brief) => {
                const colors = YEAR_COLOR[brief.year] ?? { bg: 'bg-gray-50 border-gray-200', badge: 'bg-gray-100 text-gray-600', dot: 'bg-gray-400' };
                return (
                  <div key={brief.year} className={`rounded-2xl border p-4 ${colors.bg}`}>
                    ...
                    <p className="text-sm font-semibold text-gray-900 mb-1">{brief.theme}</p>
